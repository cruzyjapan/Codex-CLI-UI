...
async function loadProjectConfig() {
  const configPath = path.join(process.env.HOME, '.codex', 'project-config.json');
  try {
...
async function extractProjectDirectory(projectName) {
  ...
  const projectDir = path.join(process.env.HOME, '.codex', 'projects', projectName);
  ...
  try {
    ...
    if (jsonlFiles.length === 0) {
      ...
      try {
        // Handle custom base64url encoding used by codex
...
async function getProjects() {
  const codexDir = path.join(process.env.HOME, '.codex', 'projects');
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        const projectPath = path.join(codexDir, entry.name);
...
async function addProjectManually(projectPath, displayName = null) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Failed to create directory: ${absolutePath}`);
    }
  }
