...
function ToolsSettings({ isOpen, onClose }) {
  ...
  // GPT-5 is the main model with reasoning effort levels
  const availableModels = [
    { value: 'gpt-5', label: 'GPT-5', description: 'Latest OpenAI model - adjustable with reasoning effort' },
    { value: 'gpt-4o', label: 'GPT-4o', description: 'GPT-4 Omni model' },
    { value: 'gpt-4o-mini', label: 'GPT-4o Mini', description: 'Lightweight and fast GPT-4o' },
    { value: 'gpt-4-turbo', label: 'GPT-4 Turbo', description: '128k context support' },
  ...
  const fetchMcpServers = async () => {
    try {
      ...
      // First try to get servers using Codex CLI
  ...
  const saveMcpServer = async (serverData) => {
    try {
      ...
      // Use Codex CLI to add the server
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to save server via Codex CLI');
        }
      } else {
  ...
  const deleteMcpServer = async (serverId, scope = 'user') => {
    try {
      ...
      // Use Codex CLI to remove the server
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          throw new Error(result.error || 'Failed to delete server via Codex CLI');
        }
      } else {
        const error = await response.json();
  ...
  const loadSettings = async () => {
    try {
      ...
      if (savedSettings) {
        ...
        setSelectedModel(settings.selectedModel || 'gpt-5');
  ...
  const saveSettings = () => {
    ...
    try {
      ...
      localStorage.setItem('codex-tools-settings', JSON.stringify(settings));
      
      // Trigger storage event for current window
      window.dispatchEvent(new StorageEvent('storage', {
        key: 'codex-tools-settings',
        newValue: JSON.stringify(settings),
        oldValue: localStorage.getItem('codex-tools-settings'),
            ...
            <div className="space-y-4">
              ...
              <div className="bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800 rounded-lg p-4">
                <div className="space-y-4">
                  ...
                  {/* Show reasoning effort only for GPT-5 */}
                  {selectedModel === 'gpt-5' && (
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-2">
                        ...
                        {reasoningEffortLevels.map(level => (
                          ...
                          >
                            <input
                              ...
                              value={level.value}
                              checked={reasoningEffort === level.value}
                  ...
                  <label className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-blue-700 dark:text-blue-300">
                        Play a sound when Codex responds
                      </div>
                    </div>
                  </label>
                  {enableNotificationSound && (
                    <button
                      onClick={async () => {
                        ...
                        const currentSettings = JSON.parse(localStorage.getItem('codex-tools-settings') || '{}');
                        localStorage.setItem('codex-tools-settings', JSON.stringify({
                          ...currentSettings,
                          enableNotificationSound: true
                        }));
                        playNotificationSound();
                        // Restore original settings
                        localStorage.setItem('codex-tools-settings', JSON.stringify(currentSettings));
                      }}
