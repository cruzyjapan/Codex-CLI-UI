 ...
 * 3. codex-complete handler - Marks session as inactive when conversation finishes
  ...
  return (
    ...
    >
      {message.type === 'system' ? (
        ...
        /* Codex/Error messages on the left */
          ...
          {!isGrouped && (
            <div className="flex items-center space-x-3 mb-2">
              ...
              <div className="text-sm font-medium text-gray-900 dark:text-white">
                {message.type === 'error' ? 'Error' : 'Codex'}
              </div>
            </div>
          )}
                                    ...
                                    {selectedOption && (
                                      <div className="bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3">
                                        <p className="text-amber-900 dark:text-amber-100 text-sm font-medium mb-1">
                                          ✓ Codex selected option {selectedOption}
                                        </p>
                      ...
                      return (
                        <>
                          ...
                          <div className="bg-amber-100 dark:bg-amber-800/30 rounded-lg p-3">
                            ...
                            <p className="text-amber-800 dark:text-amber-200 text-xs">
                              Please select an option in your terminal where Codex is running.
                            </p>
                          </div>
                        </>
                      );
...
function ChatInterface({ selectedProject, selectedSession, ws, sendMessage, messages, onFileOpen, onInputFocusChange, onSessionActive, onSessionInactive, onReplaceTemporarySession, onNavigateToSession, onShowSettings, autoExpandTools, showRawParameters, autoScrollToBottom }) {
  ...
  const [isYoloMode, setIsYoloMode] = useState(() => {
    try {
      const settings = JSON.parse(localStorage.getItem('codex-tools-settings') || '{}');
  ...
  const [selectedModel, setSelectedModel] = useState(() => {
    try {
      const settings = JSON.parse(localStorage.getItem('codex-tools-settings') || '{}');
      return settings.selectedModel || 'codex-2.5-flash';
    } catch (e) {
      return 'codex-2.5-flash';
    }
  });
  ...
  useEffect(() => {
    const checkSettings = () => {
      try {
        ...
        setSelectedModel(settings.selectedModel || 'codex-2.5-flash');
      } catch (e) {
        setIsYoloMode(false);
        setSelectedModel('codex-2.5-flash');
      }
    };
    ...
    if (messages.length > 0) {
      ...
      switch (latestMessage.type) {
        case 'session-created':
          // New session created by Codex CLI - we receive the real session ID here
        ...
        case 'codex-response':
          ...
          // Handle Codex CLI session duplication bug workaround:
          // When resuming a session, Codex CLI creates a new session instead of resuming.
          ...
          if (latestMessage.data.type === 'system' && 
            ...
            // Debug - Codex CLI session duplication detected
        ...
        case 'codex-error':
          // console.log('Codex error, setting isLoading to false:', latestMessage.error);
        ...
        case 'codex-complete':
          // console.log('Codex completed, setting isLoading to false');
          setIsLoading(false);
          setCanAbortSession(false);
          setCodexStatus(null);
        ...
        case 'codex-status':
          // Handle Codex working status messages
          // Debug - Received codex-status message
  ...
  const handleSubmit = async (e) => {
    ...
    setIsUserScrolledUp(false); // Reset scroll state so auto-scroll works for Codex's response
    ...
    const getToolsSettings = () => {
      try {
        const savedSettings = localStorage.getItem('codex-tools-settings');
        if (savedSettings) {
          ...
          return {
            ...
            selectedModel: settings.selectedModel || 'codex-2.5-flash'
          };
        }
      } catch (error) {
      ...
      return {
        ...
        selectedModel: 'codex-2.5-flash'
      };
    };

    const toolsSettings = getToolsSettings();

    // Send command to Codex CLI via WebSocket with images
    if (toolsSettings.selectedModel === 'gpt-5') {
      console.log('[ChatInterface] Sending GPT-5 request with reasoning effort:', toolsSettings.reasoningEffort || 'medium');
    }
    ...
    sendMessage({
      ...
      options: {
        ...
        reasoningEffort: toolsSettings.reasoningEffort || 'medium',
  ...
  const handleKeyDown = (e) => {
    ...
    // Disabled for Codex - no permission modes
    /*
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p>Select a project to start chatting with Codex</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
        ...
        {isLoadingSessionMessages && chatMessages.length === 0 ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-gray-500 dark:text-gray-400 px-6 sm:px-4">
              <p className="font-bold text-lg sm:text-xl mb-3">Start a conversation with Codex</p>
          ...
          <div className="chat-message assistant">
            <div className="w-full">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <div className="text-sm font-medium text-gray-900 dark:text-white">Codex</div>
      ...
      <div className={`p-2 sm:p-4 md:p-6 flex-shrink-0 ${
        ...
        {/* Codex Working Status - positioned above the input form */}
        <CodexStatus 
          status={codexStatus}
          isLoading={isLoading}
        ...
        {/* Codex Mode Indicator - Above input */}
          ...
          <div className="flex items-center justify-center gap-3">
            <div className={`px-4 py-1.5 rounded-lg text-sm font-medium border transition-all duration-200 ${
              ...
              <div className="flex items-center gap-2">
                ...
                <span>{isYoloMode ? 'Full Auto Mode' : 'Codex Default'}</span>
        ...
        <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
          ...
          <div {...getRootProps()} className={`chat-input-container relative bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500 focus-within:border-blue-500 transition-all duration-200 ${isTextareaExpanded ? 'chat-input-expanded' : ''}`}>
            ...
            <textarea
              ...
              placeholder="Ask Codex to help with your code... (@ to reference files)"
              disabled={isLoading}
              rows={1}
