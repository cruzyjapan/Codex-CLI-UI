...
Codex CLI（AI支援コーディング用の高度なCLI）のデスクトップ・モバイル対応UIです。ローカルまたはリモートで使用でき、Codex CLIのアクティブなプロジェクトとセッションを表示し、CLIと同じように変更を加えることができます。どこでも動作する適切なインターフェースを提供します。
...
- **レスポンシブデザイン** - デスクトップ、タブレット、モバイルでシームレスに動作し、モバイルからもCodex CLIを使用可能
- **インタラクティブなチャットインターフェース** - Codex CLIとのシームレスな通信のための組み込みチャットインターフェース
- **統合シェルターミナル** - 組み込みシェル機能によるCodex CLIへの直接アクセス
...
- Codex CLIがインストールされ、設定済みであること
  - ソースからインストールまたはバイナリをダウンロード
  - `codex`コマンドがPATHに通っていることを確認
  - または`CODEX_PATH`環境変数にcodex実行ファイルのフルパスを設定
...
5. **ブラウザを開く：**
   - 開発環境: `http://localhost:6009`

## セキュリティとツール設定

**🔒 重要なお知らせ**: すべてのOpenAI Codexツールは**デフォルトで無効**になっています。これにより、潜在的に有害な操作が自動的に実行されることを防ぎます。

### ツールの有効化

OpenAI Codexの全機能を使用するには、手動でツールを有効にする必要があります：
...
YOLOモード（「You Only Live Once」）は、OpenAI Codexの `--yolo` フラグに相当し、すべての確認プロンプトをスキップします。このモードは作業を高速化しますが、注意して使用してください。
...
#### プロジェクト管理
UIは `~/.codex/projects/` からOpenAI Codexプロジェクトを自動的に検出し、以下を提供します：
- **ビジュアルプロジェクトブラウザー** - メタデータとセッション数を含むすべての利用可能なプロジェクト
- **プロジェクトアクション** - プロジェクトの名前変更、削除、整理
- **スマートナビゲーション** - 最近のプロジェクトとセッションへのクイックアクセス

#### チャットインターフェース
- **レスポンシブチャットまたはOpenAI Codexを使用** - 適応されたチャットインターフェースを使用するか、シェルボタンを使用してOpenAI Codexに接続できます
- **リアルタイム通信** - WebSocket接続によるOpenAI Codexからのストリームレスポンス
...
```
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   フロントエンド   │    │   バックエンド    │    │  OpenAI Codex   │
│   (React/Vite)  │◄──►│ (Express/WS)    │◄──►│  統合           │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```

### バックエンド（Node.js + Express）
- **Expressサーバー** - 静的ファイルサービングを備えたRESTful API（ポート: 6008）
- **WebSocketサーバー** - チャットとプロジェクト更新のための通信
- **OpenAI Codex統合** - プロセスの生成と管理
- **セッション管理** - JSONLパースと会話の永続化
- **ファイルシステムAPI** - プロジェクト用のファイルブラウザーを公開
- **認証システム** - セキュアなログインとセッション管理（SQLiteデータベース: codexcliui_auth.db）
...
### ポート設定
- **APIサーバー**: 6008番ポート（デフォルト）
- **フロントエンド開発サーバー**: 6009番ポート（デフォルト）
- これらのポートは`.env`ファイルで変更可能です

### データベース設定

#### 初期設定とテーブル構造
- **データベースファイル**: `server/database/codexcliui_auth.db`
- **データベースタイプ**: SQLite 3
- **初期化**: サーバー起動時に自動的に作成・初期化されます

#### ユーザーテーブル詳細

**テーブル名**: `codexcliui_users`
...
**インデックス**:
- `idx_codexcliui_users_username`: ユーザー名での高速検索用
- `idx_codexcliui_users_active`: アクティブユーザーのフィルタリング用
...
**解決策**:
- OpenAI Codexが適切にインストールされていることを確認（`npm i -g @openai/codex` または `brew install codex`）
- 少なくとも1つのプロジェクトディレクトリで`codex`コマンドを実行して初期化
- `~/.codex/projects/`ディレクトリが存在し、適切な権限があることを確認
...
**主な変更点:**
- Claude Code UIをベースにCodex CLI用に完全再構築
- 認証システムの追加（SQLiteベース）
- Codex CLI専用の機能と統合
- 日本語対応の強化
- Codex CLIワークフローに合わせた完全なUI再設計
